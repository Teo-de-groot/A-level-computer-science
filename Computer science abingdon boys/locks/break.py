...
class Breakthrough():
    ...
    def __UseMulligan(self):
        while self.__Hand.GetNumberOfCards() > 0:
            card_num = self.__Hand.GetCardNumberAt(0)
            self.__MoveCard(self.__Hand, self.__Deck, card_num)
            

        while self.__Sequence.GetNumberOfCards() > 0:
            card_num = self.__Sequence.GetCardNumberAt(0)
            self.__MoveCard(self.__Sequence, self.__Deck, card_num)
            

        while self.__Discard.GetNumberOfCards() > 0:
            card_num = self.__Discard.GetCardNumberAt(0)
            self.__MoveCard(self.__Discard, self.__Deck, card_num)
        ...
        if len(self.__Locks) > 0:
            ...
            while not self.__GameOver:
                ...
                while not self.__LockSolved and not self.__GameOver:
                    ...
                    print("Cards left:" , self.__Deck.GetNumberOfCards())
                    print(self.__Hand.GetCardDisplay())
                    MenuChoice = self.__GetChoice()
                    if MenuChoice == "P" and not self.__CurrentLock.getPeekUsed():
                        print(self.__Deck.GetCardDescriptionAt(1), "\n", self.__Deck.GetCardDescriptionAt(2), "\n" ,self.__Deck.GetCardDescriptionAt(3), "\n")
                        self.__CurrentLock.PeekUsed()
                    elif MenuChoice == "D":
                        ...
                        if not self.MulliganUsed():
                            ...
                            print("Mulligan already used")
                    elif MenuChoice == "Q":
                        cards_left = self.__Deck.GetNumberOfCards()
                        self.__Score += cards_left
                        self.__LockSolved = True  
                        self.__GameOver = True    
                        print("You received ", {cards_left}, "bonus points for remaining cards.")
    ...
    def __CreateStandardDeck(self):
        for Count in range((random.randrange(1,2443))):
...
class CardCollection():
    ...
    def Shuffle(self):
        ...
        half = deck_size // 2    
        half1 = self._Cards[:half]
        half2 = self._Cards[half:]
        while len(half1) > 0 or len(half2) > 0:
            num = random.randint(0, 5)
            for _ in range(num):
                if len(half1) > 0:
                   new_deck.append(half1.pop(0))
            num = random.randint(0, 5)
            for _ in range(num):
                if len(half2) > 0:
                   new_deck.append(half2.pop(0)) 
